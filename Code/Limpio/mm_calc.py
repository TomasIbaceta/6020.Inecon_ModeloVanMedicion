...
def mm_calc_run_alg(exceldata, parametros_localidad, scenario:dict):
    ...
    df_referenciasClase=exceldata["REFERENCIA DECAIMIENTO"]
    ...
    data["TipoMedidorBase"] = data['CLASE'].astype(str) + '-' + data['DIAMETRO_MEDIDOR'].astype(str)
    df_referenciasClase["TipoMedidorBase"] = df_referenciasClase['CLASE_BASE'].astype(str) + '-' + df_referenciasClase['DIAMETRO_MEDIDOR_BASE'].astype(str)
    #----- SNIPPET BEGIN HERE ----
    ...
    E = data.merge(df_referenciasClase, on="TipoMedidorBase", how='left')
    E = E.merge(caudal_ext, left_on="Referencia Decaimiento Base", right_on="Clase", how='left')
    ...
    # add the extra columns just at the end
    data = data.merge(df_referenciasClase, on="TipoMedidorBase", how='left')
...
def error(group,autocontrol, autocontrol_diametro_alto,  error_ultra, intervalos_caudal_bajo, intervalos_caudal_bajo_c25, flag=1):
    ...
    #     grupo = group["Referencia Decaimiento Base"].tolist()[0]
    # else:
    #     # grupo = group["Curva proy"].tolist()[0]
    #     grupo = group["Referencia Decaimiento Reemplazo"].tolist()[0]
        
    diametro = group["Diametro_max"].values[0]
    grupo = group["Referencia Decaimiento Base"].values[0] if flag == 1 else group["Referencia Decaimiento Reemplazo"].values[0]
